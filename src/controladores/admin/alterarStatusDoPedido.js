...
const alterarStatusDoPedido = async (req, res) => {
    ...
    try {
        ...
        if (pedidosPorNome.length === 0) {
            return res.status(404).json({ mensagem: "Nenhum pedido encontrado com esse nome." });
        }

        if (pedidosPorNome[0].status === "concluído") {
            return res.status(404).json({ mensagem: "Não é possível alterar o status de um pedido concluído." });
        }
        const idPedido = pedidosPorNome[0].id

        await knex('pedidos').where({ id: idPedido }).update({ status }).returning('*')
